...
class RealtimeAnalyzer {
    ...
    func analyse(with buffer: AVAudioPCMBuffer) -> [[Float]] {
        ...
        for (index, amplitudes) in channelsAmplitudes.enumerated() {
            ...
            var spectrum = bands.map {
                findMaxAmplitude(for: $0, in: weightedAmplitudes, with: Float(buffer.format.sampleRate)  / Float(self.fftSize)) * 9
            }
    ...
    /*
     vDSP figital signal processing
     PCM 脉冲编码调制，是一种模拟信号数字化的方法
     frame 音频帧，一般取20ms，40ms的音频数据为一帧
     packet 网络传输打包，一般包括一个frame或多个frame或一个frame的一部分
     sample 音频采样数据
     channel 声道，或通道 （mono 单声道， Stereo 立体声）
     stream 音频流
     */
    private func fft(_ buffer: AVAudioPCMBuffer) -> [[Float]] {
        ...
        // isInterleaved : 声道混合
        if isInterleaved {
            // deinterleaved L1 R1 L2 R2 L3 R3 ===> channel[0]: L1 L2 L3 channel[1]: R1 R2 R3
            ...
            for i in 0 ..< channelCount {
                // stride 正向跳跃遍历by:channelCount to:< interleavedData.count
        ...
        for i in 0 ..< channelCount {
            ...
            // 3.将实体包装成FFT要求的复数fftInOut，即是输入也是输出
            ...
            channelAmplitudes[0] = channelAmplitudes[0]/2 // 直流分量的振幅需再除2
    ...
    private func highlightWaveform(spectrum: [Float]) -> [Float] {
        ...
        //   可以随意修改，个数需要奇数
        let weights: [Float] = [3, 5, 7, 9, 7, 5, 3]
